...
<template>
  <div class="container pt-3">
    <header class="jumbotron">
      <h3>
        <strong>Golden Memories</strong> Post a photo
      </h3>
    </header>
    ...
    <div class="row py-4">
        <div class="col-lg-8 mx-auto">
          ...
          <Form @submit="uploadPhoto" :validation-schema="photoSchema">
            <div v-if="!successful">
              ...
              <div class="input-group mt-3 px-2 py-2 rounded-pill bg-white shadow-sm">
                  ...
                  <div class="input-group-append">
                      <label for="upload" class="btn btn-light m-0 rounded-pill px-4"> 
                        ...
                        <span class="text-uppercase font-weight-bold text-muted">Choose an image</span>
                      </label>
                  </div>
              </div>
              ...
              <div class="input-group mt-3 px-2 py-2 rounded-pill bg-white shadow-sm">
                  <Field id="message" name="message" type="text" class="form-control border-0" placeholder="Type a message" />
              </div>
              <b><ErrorMessage name="message" class="error-feedback small text-danger" /></b>
...
export default {
  ...
  methods: {
    uploadPhoto(photoData) {
      ...
      formData.append("image", photoData.image[0]);
      formData.append("message", photoData.message);

      PhotoService.uploadPhoto(formData).then(
        (response) => {
          this.message = response.data.message;
          this.successful = true;
          this.loading = false;
          this.imageData = null;
          setTimeout(() => this.$router.push('/gallery'), 2000);
        },
        (error) => {
          this.message =
            (error.response &&
